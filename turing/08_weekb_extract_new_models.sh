...
# Fail-fast GPU sanity check. Previous run silently fell back to CPU
# (driver mismatch) and produced ~14-day ETAs. We catch that here.
python -u - <<'PYCHECK'
...
if dt > 500:
    print(f"FATAL: GPU op took {dt:.0f}ms (expected <100ms). "
          f"Likely silent CPU fallback. Job aborted.", file=sys.stderr)
    sys.exit(2)
PYCHECK
if [ $? -ne 0 ]; then
    echo "GPU SANITY CHECK FAILED. Aborting."
    exit 1
fi
