#!/bin/bash
# =============================================================================
# Single-model probe job: LLaVA-OneVision-7B
# =============================================================================
# Lighter resource ask than the array job — easier to slot into the queue when
# cluster is congested. Submit independently:
#
#     sbatch turing/10_probe_llava_ov.sh
#
# Same per-model logic as 08b_weekb_array.sh but pinned to one model.
# Failure exits non-zero so the wrapper aggregator's `afterany` still fires.
# =============================================================================

#SBATCH -J probe-llava
#SBATCH -p quick
#SBATCH -N 1
#SBATCH -n 4
#SBATCH --mem=48G
...
readonly MODEL="llava-onevision-7b"

# Activate env (with `set -u` guard for older conda versions)
set +u
...
# ----- GPU sanity (catches silent CPU fallback / driver mismatch) -----
python -u - <<'PYCHECK'
...
# ----- VAL-split probing (the actual data the predictor needs) -----
echo ""
...
# ----- Sample sanity check -----
# Count NEWLY-processed PLUS already-CACHED samples (the latter are loaded
# from disk in the resume path and are equally valid). Earlier version
# only counted `processed` and false-FAIL'd whenever resuming from cache.
