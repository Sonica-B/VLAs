#!/bin/bash
# =============================================================================
# Single-model probe job: Idefics3-8B-Llama3 (Pixtral replacement)
# =============================================================================
# Idefics3 (HuggingFaceM4/Idefics3-8B-Llama3) — 8B params, SigLIP-SO400M +
# pixel-shuffle (r=2, ~4x token compression) + Llama 3.1 8B. Apache-2.0.
# Ships in transformers >=4.46 — no env upgrade needed.
#
# Scientific role: MID-COMPRESSION data point (~4x). Fills the gap between
# InternVL3-8B (2.4x) and Gemma4-E4B (114x) in the PhysLens-Predict LOO
# regression — strengthens interpolation across the compression range.
#
# Replaces Pixtral-12B which had unfixable transformers 4.46.x bugs
# (variable-resolution multi-image processor → list[Tensor] forward crash).
#
#     sbatch turing/13_probe_idefics3.sh
# =============================================================================

#SBATCH -J probe-idefics3
#SBATCH -p quick
#SBATCH -N 1
#SBATCH -n 4
#SBATCH --mem=48G
